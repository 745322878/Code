    ...
    public sealed partial class MainView : Page
    {
        ...
        public async  void Init()
        {
         ...
         private  void btn_PointerExited(object sender, PointerRoutedEventArgs e)
         {
             ...
             BitmapImage _bitmap =(BitmapImage)imagebrush.ImageSource;
             for (int i = 0; i < App.filmPhoto.Count; i++)
             {
                 Uri uri = new Uri(App.filmPhoto[i]);
                 BitmapImage bitmap = new BitmapImage(uri);
                 if (_bitmap.UriSource.ToString()==bitmap.UriSource.ToString())
                 {
                     count = i;
                     break;
                 }
             ...
             else if(angle>=225 && angle<315)           //从上往下
             {
                  count++;
                 if (count == App.filmPhoto.Count)
                     count = 0;   
             }
             Uri uri1 = new Uri(App.filmPhoto[count]);
             BitmapImage bitmap1 = new BitmapImage(uri1);
             imagebrush.ImageSource = bitmap1;
         }
